...
cube.read<-function(
  ...
  for (ivar in 1:length(variable)) {
    ...
    if (y1==y2) {
      ...
      if (y1+1<y2) {
        for (y in (y1+1):(y2-1)) {
          nc<-RNetCDF::open.nc(file.path(cube$base.dir,"data",variable[ivar],paste0(y,"_",variable[ivar],".nc")))
      ...
      #Read from last Year
      nc<-RNetCDF::open.nc(file.path(cube$base.dir,"data",variable[ivar],paste0(y2,"_",variable[ivar],".nc")))
